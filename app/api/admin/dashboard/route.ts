...
export async function GET(request: Request) {
  ...
  const [orders, products, customers, recentOrders, lowStock] = await prisma.$transaction([
  ...
  return NextResponse.json({ metrics: { todayRevenue: revenue, todayOrders: todayOrders.length, totalProducts: products, totalCustomers: customers, lowStock: lowStock.length }, statusCounts, recentOrders, lowStock });
}