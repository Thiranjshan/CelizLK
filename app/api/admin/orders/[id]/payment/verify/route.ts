...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const messages: Record<string, string> = { ORDER_NOT_FOUND: 'Order not found.', INVALID_PAYMENT_METHOD: 'This order does not use bank transfer.', PAYMENT_NOT_FOUND: 'Payment not found.', PAYMENT_ALREADY_PAID: 'Payment is already confirmed.', PAYMENT_ALREADY_REJECTED: 'Payment has already been rejected.', INVALID_PAYMENT_STATE: 'Payment is not eligible for confirmation.', AMOUNT_MISMATCH: 'Submitted amount does not match the order total.', PAYMENT_CONFLICT: 'Payment changed by another admin. Refresh and try again.', ORDER_CONFLICT: 'Order changed by another admin. Refresh and try again.' };
